...
class SR_Checkin():
    ...
    #api
    Referer_url     = 'https://webstatic.mihoyo.com/bbs/event/signin/hkrpg/index.html?bbs_auth_required=true&act_id={}&bbs_auth_required=true&bbs_presentation_style=fullscreen&utm_source=bbs&utm_medium=mys&utm_campaign=icon'
    Sign_url        = 'https://api-takumi.mihoyo.com/event/luna/sign' #POST json
    Check_url       = 'https://api-takumi.mihoyo.com/event/luna/info?act_id={}&region={}&uid={}'

    #value
    Act_id          = 'e202304121516551'
    Game_biz        = 'hkrpg_cn'

    def sr_sign(self,region,uid):
        sign_data = {
            "act_id": self.Act_id,
            "region": region,
            "uid": uid
        }
        log.info('崩坏 星穹铁道: 正在为开拓者「{}」签到'.format(uid))
    ...
    def run(self,list):
        ...
        for i in range(3):
            sign_data = self.sr_sign(region,uid) #签到并返回数据
            try:
                #从专门的api检查签到结果
                ...
                if "OK" in sign_data["message"]:
                    if data["data"]["is_sign"]: # "is_sign" 为布尔值
                        log.info('崩坏 星穹铁道: 签到成功')
                        break
                    else:
                        log.info('崩坏 星穹铁道: 第 {} 次请求成功但并未签上'.format(i + 1))
                elif "已签到" in sign_data["message"]:
                    log.info('崩坏 星穹铁道: 重复签到')
                    break
                else:
                    log.warning(sign_data)
                    log.warning('崩坏 星穹铁道: 签到出现错误,请及时检查')
                    break
            except:
                #直接输出签到api返回的签到结果
                log.warning(data)
                ...
                if "OK" in sign_data["message"]:
                    log.info('崩坏 星穹铁道: 签到成功')
                elif "开拓者,你已经签到过了" in sign_data["message"]:
                    log.info('崩坏 星穹铁道: 重复签到')
                else:
                    log.warning(sign_data)
                    log.warning('崩坏 星穹铁道: 签到出现错误,请及时检查')
...
def StartRun():
    ...
    #游戏每日签到
    if Enable["BH2"] or Enable["BH3"] or Enable["YS"] or Enable["SR"]:
        for list in GetAllRoles()["data"]["list"]:
            ...
            if list["game_biz"] == "hkrpg_cn" and list["game_uid"] not in Game_BlackList["SR"] and Enable["SR"]:
                SR_Checkin().run(list) #崩坏 星穹铁道福利补给
